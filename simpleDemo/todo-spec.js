...
describe('End2End swag labs web application with protractor', function () {
  it('should app start and login correctly', function () {
    ...
    //start application
    browser.get('http://cqcehap001.qcenv.idemia.local:8000/vps-hub/login');

    //fill username and passwor then click login button
    browser.driver.findElement(by.id('login-username-input')).sendKeys('super');
    browser.driver.findElement(by.id('login-password-input')).sendKeys('super');
    browser.driver.findElement(by.id('login-button')).click();
    //verify if login successfully
    expect(browser.driver.findElement(by.xpath('//span[contains(text(),'القائمة الرئيسية')]')).getText()).toEqual('القائمة الرئيسية');
  });
